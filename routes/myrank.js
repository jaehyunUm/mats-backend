...
router.get('/ranking/:testId', verifyToken, async (req, res) => {
  ...
  const { dojang_code } = req.user; // 미들웨어에서 추출한 도장 코드
  const dojangOnly = req.query.dojang_code === 'true'; // 내 도장 필터 여부
  ...
  try {
    ...
    const params = dojangOnly ? [testTemplateId, dojang_code] : [testTemplateId];
